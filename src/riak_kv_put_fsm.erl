...
validate(timeout, StateData0 = #state{from = {raw, ReqId, _Pid},
    ...
    if
        ...
        true ->
            ...
            RObj = riak_object:apply_updates(RObj1),
            {VNodeReturnBody, ReturnBody} =
            ...
            PutCore = riak_kv_put_core:init(N, W, PW, DW,
                                            ...
                                            ReturnBody,
                                            IdxType),
            VNodeOpts0 = handle_options(Options, []),
            VNodeOpts = fixup_opts(VNodeReturnBody, VNodeOpts0),
            StateData = StateData0#state{n=N,
                                         w=W,
...
execute_local(StateData=#state{robj=RObj, req_id = ReqId,
                               ...
                               trace = Trace,
                               starttime = StartTime}) ->
    ?DTRACE(Trace, ?C_PUT_FSM_EXECUTE_LOCAL, [], [atom2list(Node)]),
    TRef = schedule_timeout(Timeout),
    riak_kv_vnode:coord_put(CoordPLEntry, BKey, RObj, ReqId, StartTime, VnodeOptions),
    StateData2 = StateData#state{robj = RObj, tref = TRef},
...
%% N.B. Not actually a state - here in the source to make reading the flow easier
execute_remote(StateData0=#state{robj=RObj, req_id = ReqId,
    ...
    StateData = 
        case Trace of
            true ->
                ...
                add_timing(execute_remote, StateData0);
            _ ->
                StateData0
        end,
...
fixup_opts(ReturnBody, Opts) ->
    Find = lists:keyfind(returnbody, 1, Opts),
    case {Find, ReturnBody} of
        {false, _} ->
            [{returnbody, ReturnBody}|Opts];
        {{_, true}, false} ->
            lists:keyreplace(returnbody, 1, Opts, {returnbody, true});
        {{_, false}, true} ->
            lists:keyreplace(returnbody, 1, Opts, {returnbody, false});
        _ ->
            Opts
    end.
